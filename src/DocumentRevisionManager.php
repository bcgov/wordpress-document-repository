...
class DocumentRevisionManager {
    ...
    public function register_meta_keys_for_revisions(array $keys): array {
        ...
        // Future: add other meta fields here, e.g.
        // $custom_fields = get_option( 'document_repository_metadata_fields', [] );
     ...
     */
    public function maybe_create_revision_on_meta($meta_id, $post_id, $meta_key, $_meta_value): void {
    ...
    public function build_diff_string(int $post_id): string {
        ...
        if ($file_id) {
            ...
            $file_info = sprintf('%s (%s) <%s>', $file_name, $size_str ?: '0KB', $file_url);
        }
    ...
    private function trigger_revision(int $post_id): void {
        ...
        wp_update_post(['ID' => $post_id, 'post_content' => $diff_string]);
